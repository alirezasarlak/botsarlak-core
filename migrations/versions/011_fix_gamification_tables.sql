...
-- Fix user_quests table syntax error

-- Drop and recreate user_quests table with correct syntax
...
-- Fix leaderboard queries to use correct column names
-- Update leaderboard_entries table to include proper user info
ALTER TABLE leaderboard_entries 
ADD COLUMN IF NOT EXISTS user_first_name VARCHAR(100),
ADD COLUMN IF NOT EXISTS user_username VARCHAR(100);

-- Update leaderboard entries with user info
UPDATE leaderboard_entries 
SET user_first_name = u.first_name,
    user_username = u.username
FROM users u 
WHERE leaderboard_entries.user_id = u.user_id;

-- Create indexes for better performance
CREATE INDEX IF NOT EXISTS idx_user_quests_user_id ON user_quests(user_id);
CREATE INDEX IF NOT EXISTS idx_user_quests_quest_id ON user_quests(quest_id);
CREATE INDEX IF NOT EXISTS idx_user_quests_created_at ON user_quests(created_at);
...
CREATE TABLE IF NOT EXISTS token_transactions (
    id SERIAL PRIMARY KEY,
    user_id BIGINT NOT NULL REFERENCES users(user_id) ON DELETE CASCADE,
    transaction_type VARCHAR(20) NOT NULL, -- earn, spend, lock, unlock
    amount INTEGER NOT NULL,
    source VARCHAR(100), -- referral, quest, lottery, etc.
    description TEXT,
    created_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- Insert sample data
INSERT INTO daily_quests (quest_name, quest_description, quest_type, quest_target, points_reward, tokens_reward, difficulty, is_active) VALUES
('مطالعه روزانه', '30 دقیقه مطالعه کن', 'study', 30, 50, 5, 'easy', TRUE),
('دعوت دوست', 'یک دوست دعوت کن', 'referral', 1, 100, 10, 'easy', TRUE),
('ورود روزانه', 'هر روز وارد ربات شو', 'login', 1, 20, 2, 'easy', TRUE),
('تکمیل پروفایل', 'پروفایل خودت رو کامل کن', 'profile', 1, 30, 3, 'easy', TRUE),
('شرکت در رقابت', 'در یک رقابت شرکت کن', 'competition', 1, 75, 8, 'medium', TRUE)
ON CONFLICT (quest_name) DO NOTHING;
...
ON CONFLICT (lottery_name) DO NOTHING;
