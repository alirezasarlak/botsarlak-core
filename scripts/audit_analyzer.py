...
class AuditAnalyzer:
            ...
            try:
                with open(file_path, encoding="utf-8") as f:
                    ...
                    if any(
                        ...
                        if not any(
                            env_var in content
                            for env_var in ["os.getenv", "os.environ"]
                        ):
    ...
    def _check_documentation(self) -> Dict[str, Any]:
        ...
        return {"status": "pass" if not doc_issues else "warn", "issues": doc_issues}
...
def main():
    ...
    print(
        f"📊 Overall status: {'✅ PASS' if all(check.get('status') in ['pass', 'warn'] for check in results['checks'].values()) else '❌ FAIL'}"
    )
