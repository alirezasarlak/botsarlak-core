...
MAJOR_SUBJECTS: Dict[str, List[str]] = {
    ...
    "انسانی": ["ادبیات", "عربی", "فلسفه", "منطق", "دینی", "اقتصاد", "تاریخ", "جغرافیا", "روان‌شناسی"],
    "هنر": ["طراحی", "تاریخ هنر", "خلاقیت تصویری", "ترسیم فنی", "ادبیات", "دینی", "زبان"],
}

def _zwsp(): return r"[\\u200c\\s]*"
def _esc(s: str) -> str:
    e = re.escape(s)
...
def btn_rx(label: str, emoji: str = "", alts: tuple = ()):
    ...
    if emoji:
        E = _esc(emoji)
        parts += [fr"{E}{_zwsp()}{L}", fr"{L}{_zwsp()}{E}"]
    parts.append(L)
    for a in alts:
        A = _esc(a)
        if emoji:
            parts += [fr"{E}{_zwsp()}{A}", fr"{A}{_zwsp()}{E}"]
        parts.append(A)
