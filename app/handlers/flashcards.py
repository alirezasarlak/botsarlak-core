...
from app.keyboards import flash_menu, subject_menu, main_menu
from app.constants import (
    STATE_FLASHCARD_MENU, STATE_FLASHCARD_SUBJECT, STATE_FLASHCARD_QUESTION, STATE_FLASHCARD_ANSWER,
    STATE_FLASHCARD_REVIEW, STATE_FLASHCARD_EVALUATE, MAJOR_SUBJECTS, btn_rx, back_home_rx
)
...
async def menu(update, context):
    ...
    if txt.startswith("➕"):
        ...
        await update.message.reply_text("درس:", reply_markup=subject_menu(MAJOR_SUBJECTS[user['major']]))
        ...
        context.user_data['cards'] = cards; context.user_data['i']=0
...
async def answer(update, context):
    ...
    cid = create_card(user_id, context.user_data.pop('fc_subject'), context.user_data.pop('fc_q'), update.message.text)
...
async def review_show_answer(update, context):
    i = context.user_data['i']; cards = context.user_data['cards']
...
flashcards_conv = ConversationHandler(
    ...
    states={
        ...
        STATE_FLASHCARD_SUBJECT: [MessageHandler(filters.TEXT & ~filters.COMMAND, choose_subject)],
        STATE_FLASHCARD_QUESTION: [MessageHandler(filters.TEXT & ~filters.COMMAND, question)],
        STATE_FLASHCARD_ANSWER: [MessageHandler(filters.TEXT & ~filters.COMMAND, answer)],
        STATE_FLASHCARD_REVIEW: [MessageHandler(filters.TEXT & ~filters.COMMAND, review_show_answer)],
        STATE_FLASHCARD_EVALUATE: [MessageHandler(filters.TEXT & ~filters.COMMAND, review_eval)],
    },
