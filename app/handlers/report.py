...
from app.constants import (
    MAJOR_SUBJECTS,
    STATE_REPORT_NOTES,
    STATE_REPORT_SUBJECT,
    STATE_REPORT_TESTS,
    STATE_REPORT_TOPIC,
    back_home_rx,
    btn_rx,
)
from app.keyboards import main_menu, subject_menu
from app.models.users import get_user
from app.services.reports import finalize_report, start_report
...
async def entry(update, context):
    ...
    await update.message.reply_text(
        t("report_subject"), reply_markup=subject_menu(MAJOR_SUBJECTS[user["major"]])
    )
...
async def notes(update, context):
    ...
    await update.message.reply_text(
        t("report_summary", dur=total_dur, tests=total_tests, pts=pts),
...
async def finalize(update, context):
    ...
    total_dur, total_tests, pts = finalize_report(
        update.effective_user.id, session_id, entries
    )
